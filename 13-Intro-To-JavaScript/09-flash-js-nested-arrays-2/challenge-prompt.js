...
function greatestFrequency(toyInventory){
  var maxToy = "";
  var maxNum = 0;
  var toyArr = Object.entries(toyInventory);
  
  // ---------- Your Code Here ----------
  for(i = 0; i < toyArr.length; i++){
    if(maxNum < toyArr[i][1]){
      maxNum = toyArr[i][1];
      maxToy = toyArr[i][0];
    }
...
function createCustomObject(objectArr){
  ...
  for(i = 0; i < objectArr.length; i++){
    ...
    if(customToyLineObj[objectArr[i].toyLine]){
      // if toyline already exists, add using one method
      toysArr.push(objectArr[i].title);
      var newTotal = toyObj.totalToysInToyLine + objectArr[i].stock;
      toyObj = {toyLine: objectArr[i].toyLine,
                toyLineToys: toysArr,
                totalToysInToyLine: newTotal};
    }
    else {
      //  new toyline on object
      var toysArr = [objectArr[i].title];
      var toyObj = {toyLine: objectArr[i].toyLine,
                    toyLineToys: toysArr,
                    totalToysInToyLine: objectArr[i].stock};
    }

    customToyLineObj[objectArr[i].toyLine] = toyObj;
  }
  ...
  for(i = 0; i < arr.length; i++){
    ...
    for(q = 0; q < arr.length; q++){
      if(q === i){
        ...
        // loop through list to see if matches possDupe
        if(arr[q] === possDupe){
          return true
          // console.log(`${possDupe} is equal to ${arr[q]}`)
        }
      }
    }
  }
