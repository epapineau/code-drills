...
var table = d3.select("tbody")
    ...
    .html(function(d){
        var symbol;
        if(d.result === "passed"){
            symbol = '<i class="fas fa-check-circle fa-lg" style="color:Green"></i>';
        }
        else{
            symbol = '<i class="fas fa-times-circle fa-lg" style="color:Red"></i>'
        }
        return `<td>${d.prop}</td><td>${d.description}</td><td>${symbol}</td>`
    });
