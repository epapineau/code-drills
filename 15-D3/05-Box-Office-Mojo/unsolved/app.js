...
function xScale(boxOffice, chosenXAxis){
  var xLinearScale = d3.scaleLinear()
    .domain([
      d3.min(boxOffice, d => d[chosenXAxis]) * .8,
...
function updateToolTip(chosenXAxis, circlesGroup) {
  if (chosenXAxis === "domestic") {
    var label = "Domestic Film Sales:";
  }
  else {
    var label = "Overseas Film Sales:";
  }
  ...
  var toolTip = d3.tip()
    ...
    .offset([80, -60])
    .html(function(d) {
      return (`${d.film}<br>${label} ${d[chosenXAxis]}`);
    });
...
d3.csv("boxOffice.csv", function(err, boxOffice) {
  ...
  boxOffice.forEach(function(d){
    d.domestic = +d.domestic;
    d.overseas = +d.overseas;
    d.worldwide = +d.worldwide;
  });
  ...
  var labelDomestic = labelsGroup.append("text")
    .attr("x", 0)
    .attr("y", 20)
  ...
  var labelOverseas = labelsGroup.append("text")
  .attr("x", 0)
  .attr("y", 40)
